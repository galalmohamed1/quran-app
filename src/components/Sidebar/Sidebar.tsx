...
import { SurahType, AyahOfTheDayType } from "@/types/type";
import useModalIsOpen from "@/hooks/useModalIsOpen";
import Modal from "../UI/Modal";
import { setScheduleFromLS } from "@/app/rtk/slices/scheduleSlice";
import { AyahOfTheDay } from "@/api/home.services";
...
const Sidebar = () => {
	...
	async function fetchAyah() {
		try {
			const surahData = await AyahOfTheDay(randomAyahNumber);
			
			localStorage.setItem("ayahOfTheDay", JSON.stringify(surahData));
			setAyahOfTheDay(surahData);
		} catch {
			console.log(Error);
		}
	}
			...
			<div
				className={`${
					currentSchedule.completed
						? "bg-green-100 dark:bg-green-950/30 border-2 !border-green-400 dark:!border-green-600"
						: "bg-slate-50 dark:bg-slate-800/50"
				} rounded-md p-4 border-2 border-transparent text-slate-800 dark:text-slate-100 transition-colors duration-200`}
			>
				...
				<h3
					className={`font-semibold text-primary-gray dark:text-slate-200 pt-4 pb-6 w-full line-clamp-2 ${
				...
				<div className="min-h-[30px] flex justify-between items-center mt-3">
					<span className="f-c !justify-start gap-2 text-primary-gray-2 dark:text-slate-400 font-semibold">
						<BiTimeFive size={25} />
	...
	const listContent = list.length ? (
		<div className="h-[calc(100%-61px)] rounded-md hidden lg:block">
			<Input
				...
				placeholder="Search..."
			/>
			<div className="h-[280px] px-3 overflow-y-scroll bg-slate-50 dark:bg-slate-800/30 rounded-md mt-3 transition-colors duration-200">
				{listToShow.map(surah => (
					<Link
						href={`/read/surah/${surah.number}`}
						key={surah.number}
						className={`flex justify-between items-center text-primary-gray-2 dark:text-slate-300 p-2 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800/50 hover:!text-primary-color ${
	...
	return (
		<>
			...
			{pathname === "/" && (
				<div className="w-full lg:w-[350px] bg-white dark:bg-slate-900 border-r dark:border-slate-800 py-4 px-4 lg:pl-4 transition-colors duration-200">
					...
					<Card
						...
						button={
							<Link href="/schedule">
								<Button
									text="see all"
									customStyles="!bg-transparent hover:!bg-transparent !text-primary-gray dark:!text-slate-400 hover:!text-primary-color"
								/>
							</Link>
						}
					...
					<hr className="dark:border-slate-800" />
						{ayahOfTheDay?.status == "OK" && (
							<div className="w-full flex flex-col justify-between mx-0 mt-3 p-4 bg-primary-color rounded-md">
								<div>
								<h3 className="text-sm text-center text-primary-gray-3 text-opacity-50 font-semibold uppercase">
									آية يوميه
								</h3>
